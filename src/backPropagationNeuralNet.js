...
class BackPropagationNeuralNet {
  ...
  setWeights(
    ...
    const countWeights = (this.inputCount * this.hiddenCount) + (this.hiddenCount * this.outputCount) + this.hiddenCount + this.outputCount;
    ...
    let k = 0; // points into weights param
  ...
  getWeights() {
    var weightsCount = (this.inputCount * this.hiddenCount) + (this.hiddenCount * this.outputCount) + this.hiddenCount + this.outputCount;
  ...
  updateWeights (
    ...
    for (var i = 0; i < this.outputGradients.length; ++i)
    {
      ...
      this.outputGradients[i] = derivative * (tValues[i] - this.outputs[i]) // oGrad = (1 - O)(O) * (T-O)
    }
    ...
    for (var i = 0; i < this.hiddenGradients.length; ++i)
    {
      ...
      this.hiddenGradients[i] = derivative * sum; // hGrad = (1-O)(1+O) * E(outputGradients*oWts)
    }
    ...
    for (var i = 0; i < this.inputHiddenWeights.length; ++i) // 0..2 (3)
    {
      for (var j = 0; j < this.inputHiddenWeights[0].length; ++j) // 0..3 (4)
      {
        ...
        this.inputHiddenWeights[i][j] += mom * this.inputHiddenPrevWeightsDelta[i][j]; // add momentum using previous delta. on first pass old value will be 0.0 but that's OK.
    ...
    for (var i = 0; i < this.hiddenBiases.length; ++i)
    {
      ...
      this.hiddenBiases[i] += mom * this.hiddenPrevBiasesDelta[i];
    ...
    for (var i = 0; i < this.hiddenOutputWeights.length; ++i)  // 0..3 (4)
    {
      for (var j = 0; j < this.hiddenOutputWeights[0].length; ++j) // 0..1 (2)
      {
        ...
        this.hiddenOutputWeights[i][j] += mom * this.hiddenOutputPrevWeightsDelta[i][j];
    ...
    for (var i = 0; i < this.outputBiases.Length; ++i)
    {
      ...
      this.outputBiases[i] += mom * this.outputPrevBiasesDelta[i];
